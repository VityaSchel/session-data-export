...
const args = await yargs(hideBin(process.argv))
  .usage(
    'Usage: $0 [-i /path/to/session/directory] [-p yourpassword] -o /path/to/output/directory',
  )
  .option('input', {
...
await fs.copyFile(path.join(inputDirectory, 'sql/db.sqlite'), unencryptedDbPath)
...
const attchmentsDirectory = path.resolve(inputDirectory, 'attachments.noindex')
...
for (const fileId of fileNames) {
  ...
  let realFilenameWithSuffix =
    realFilename.slice(0, -path.extname(realFilename).length) +
    suffix +
    path.extname(realFilename)
  ...
  usedFileNames.set(realFilename, (usedFileNames.get(realFilename) ?? 0) + 1)
  remappedFiles++
}
console.log(
  'Successfully remapped ' + remappedFiles + ' files to their original names',
)
